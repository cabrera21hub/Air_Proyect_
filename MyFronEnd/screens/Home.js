...
const Home = () => {
  ...
  useEffect(() => {
    const fetchAirQualityData = async () => {
      ...
      try {
        const response = await axios.get(`http://192.168.1.73:8000/api/calidad_aire/${formattedDate}/`, { timeout: 10000 });
  ...
  const handleYearClick = async (year) => {
    try {
      const response = await axios.get(`http://192.168.1.73:8000/api/historical_data/${year}/`, { timeout: 10000 });
